...
url_mc = 'https://www.curseforge.com/minecraft'
url_base = [url_mc + '/mc-mods', url_mc + '/modpacks']
url_search = [url + '/search?search={key}' for url in url_base]
url_proj = [url + '/{itempid}' for url in url_base]
url_filelist = [url + '/files/all?page={page}' for url in url_proj]
url_file = [url + '/download/{itemfid}/file' for url in url_proj]
url_dep = [url + '/relations/dependencies?page={page}' for url in url_proj]
'''
...
def mod_search(key, page=1, url_search=url_search[INDEX_MOD]):
...
def mod_file_page(itempid, page=1, url_filelist=url_filelist[INDEX_MOD]):
...
def mod_file(itempid, itemfid, savePath='', progressbar=True, overwrite=None, url_file=url_file[INDEX_MOD]):
...
def mod_dep_list(itempid, url_dep=url_dep[INDEX_MOD]):
    itemlist = {}
    pagenum = 1
...
def mod_dep_page(itempid, page, url_dep=url_dep[INDEX_MOD]):
    itemlist = {}
...
def pack_search(key, page=1):
    return mod_search(key, page, url_search[INDEX_PACK])

def pack_file_page(packpid, page=1):
    return mod_file_page(packpid, page, url_filelist[INDEX_PACK])

def pack_file(packpid, packfid, savePath='', progressbar=True, overwrite=None):
    return mod_file(packpid,packfid,savePath,progressbar,overwrite,url_file[INDEX_PACK])

def pack_dep_list(packpid):
    return mod_dep_list(packpid,url_dep[INDEX_PACK])
